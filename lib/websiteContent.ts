// Website content source for the AI bot.
// The assistant uses owned site content only: static service data plus the owned blog store.

import { listOwnedBlogPosts, type OwnedBlogPost } from './ownedBlog'
...
export async function getWebsiteContent(): Promise<WebsiteContent> {
  try {
    ...
    return {
      ...
      blogPosts: getBlogPostsFromOwnedPosts(blogPosts),
    }
  } catch (error) {
...
function getServicesFromContent(): ServiceInfo[] {
  return [
    {
...
function getPricingFromContent(): PricingInfo[] {
  return [
    {
...
function getBlogPostsFromOwnedPosts(posts: OwnedBlogPost[]): BlogPost[] {
...
RECENT BLOG KNOWLEDGE:
${content.blogPosts.map(post => `- ${post.title}: ${post.excerpt}`).join('\n') || '- No owned blog posts have been generated yet.'}
